! Original author(s): Evgeniy Yakushev, Elizaveta Protsenko, Shamil Yakubov, 
!                     Jorn Bruggeman, Phil Wallhead
!-----------------------------------------------------------------------
        ...
        select type (value=>pair%value)
            class is (type_scalar)
                ...
                if (.not.success) then !Assume the value is a string
                    realvalue = sqrt(-1.d0) !"standard" way of producing NaN
            ...
            if (extend_out .eqv. .true.) then
                do ip = 1, size(model%diagnostic_variables)
                    if (model%diagnostic_variables(ip)%save) then
                        ...
                        write(10,'( 1x,f15.9 )',advance='NO') temp_matrix(i,k)
                    end if
                end do
            end if
            write(10,*)
...
!=======================================================================================================================    
    ...
    !Input variables
    integer, intent(in)                       :: i_max, i_water, k_wat_bbl, k_bbl_sed, k_max, par_max, days_in_yr, freq_turb
    ...
    real(rk), dimension(:,:), intent(out)     :: kz_bio, alpha, phi, phi_inv, tortuosity, w_b, u_b 
    real(rk), dimension(:,:,:), intent(out)   :: t, s, kz, hmix_rate, kz_mol, pF1, pF2
    ...
    real(rk)                                  :: phi_0, phi_inf, z_decay_phi, w_binf, rho_def   
